...
class MapboxCircle {
    ...
    /**
     * @param {[number,number]} center Circle center `[lng, lat]` coordinates
    ...
    constructor(center, radius, options) {
        ...
        /** @const {[number,number]} */ this._lastCenterLngLat = [center[0], center[1]];
        /** @const {[number,number]} */ this._currentCenterLngLat = [center[0], center[1]];
    ...
    _updateCircle() {
        ...
        const steps = Math.max((Math.sqrt(Math.trunc(this._currentRadius * 0.25)) * zoom ^ 2), 64);
        const unit = 'meters';

        this._circle = turfCircle(this._currentCenterLngLat, this._currentRadius, steps, unit, this.options.properties);
        this._handles = [
            turfDestination(this._currentCenterLngLat, this._currentRadius, 0, unit),
            turfDestination(this._currentCenterLngLat, this._currentRadius, 90, unit),
            turfDestination(this._currentCenterLngLat, this._currentRadius, 180, unit),
            turfDestination(this._currentCenterLngLat, this._currentRadius, -90, unit)
        ];

        if (this.options.statusEl) {
            this.options.statusEl.innerHTML = ('Center: LngLat(' + this._currentCenterLngLat + ') / Radius: ' +
                                               this._currentRadius + ' meters / Bounds: ' + this.getBounds() +
                                               ' / Steps: ' + steps + ' / Instance ID: ' + this._instanceId);
        }
    }
