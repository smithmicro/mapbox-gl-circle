...
class MapboxCircle {
    ...
    /**
     * Return `true` if a position exists in the circle.
     * @param {mapboxgl.Map} [map]
     * @param {{lat: number, lng: number}} position
     * @return {boolean}
     * @private
     */
    _contains(map, position) {
        map = map || this.map;
        let positionInCircle = map.queryRenderedFeatures(event.point, {layers: [this._circleFillId]});
        return positionInCircle.length !== 0;
    }
    ...
    _onCenterHandleMouseLeave() {
        if (this._centerDragActive) {
            ...
            this._resetHandles(this._circleCenterHandleId);
            if (this._contains(this.map, event.position)) {
     ...
     * @private
     */
    _onRadiusHandlesMouseLeave(event) {
    ...
    /**
     * Set pointer cursor when entering circle fill
     * @param {MapMouseEvent} event
     * @private
     */
    static _onCircleFillMouseEnter(event) {
     ...
     * @private
     */
    static _onCircleFillMouseLeave(event) {
    ...
    _bindCircleFillListeners(map) {
        ...
        map.on('mouseenter', layerId, MapboxCircle._onCircleFillMouseEnter);
        map.on('mouseleave', layerId, MapboxCircle._onCircleFillMouseLeave);
    }
    ...
    _unbindCircleFillListeners(map) {
        ...
        map.off('mouseenter', layerId, MapboxCircle._onCircleFillMouseEnter);
        map.off('mouseleave', layerId, MapboxCircle._onCircleFillMouseLeave);
    }
