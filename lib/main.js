...
class MapboxCircle {
    ...
    /**
     * Returns true if cursor point is inside maparea
     * @param {{x: number, y: number}} point
     * @return {boolean}
     * @private
     */
    _pointInsideMap(point) {
        const canvas = this.map.getCanvas();
        return (point.x > 0 && point.x < canvas.offsetWidth) && (point.y > 0 && point.y < canvas.offsetHeight);
    }
    ...
    _onCenterHandleMouseUpOrMapMouseOut(event) {
        if (event.type === 'mouseout' && this._pointInsideMap(event.point)) {
            /* This happens when the cursor enters the area of a marker */ return;
        }
