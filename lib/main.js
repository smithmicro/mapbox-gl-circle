...
class MapboxCircle {
     ...
     */
    get _circleSourceId() {
        return 'circle-' + this._instanceId;
    }
    ...
    */
    ...
    onMouseMove(event) {
        ...
        let radiusHandles = this.map.queryRenderedFeatures(event.point, {
            layers: [this._circleRadiusHandlesId]
        });

        if (centerHandle.length || radiusHandles.length) {
            this.map.getCanvas().style.cursor = 'pointer';
            let mouseDownHandler = centerHandle.length ? this.onCenterHandleMouseDown : this.onRadiusHandleMouseDown;
    ...
    addTo(map) {
        ...
        this.map.on('load', () => {
            this.map.addSource(this._circleSourceId, {
                type: 'geojson',
