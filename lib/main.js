 ...
 * @public
 */
class MapboxCircle extends EventEmitter {
    /**
    ...
    set radius(newRadius) {
        this._radius = Math.min(Math.max(this._minRadius, newRadius), this._maxRadius);
        this._updateCircle();
        this._animate();
    }
    ...
    constructor(center, radius, options) {
        ...
        /** @type {Array<number>} */ this._lastCenter = center;
        ...
        /** @const {number} */ this._radius = Math.round(radius);
    ...
    _updateCircle() {
        ...
        if (this.options.statusEl) {
            this.options.statusEl.innerHTML = ('Center: ' + this._center + ' / Radius: ' + this._radius +
    ...
    _onCenterHandleMouseUp() {
        ...
        if (this.center[0] !== this._lastCenter[0] && this.center[1] !== this._lastCenter[1]) {
            this.emit('centerchanged', this);
        }
    }
    ...
    /**
     * Update _lastCenter on `centerchanged` event.
     * @private
     */
    _onCenterChanged() {
    ...
    addTo(map) {
        const addCircleAssetsOnMap = () => {
            ...
            if (this.options.editable) {
                ...
                this.addListener('centerchanged', this._onCenterChanged);
                this.addListener('radiuschanged', this._onRadiusChanged);
    ...
    remove() {
        if (this.options.editable) {
            ...
            this.removeListener('radiuschanged', this._onRadiusChanged);
            this.removeListener('centerchanged', this._onCenterChanged);
