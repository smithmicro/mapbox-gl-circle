...
class MapboxCircle {
    ...
    /**
     * Returns true if a point exists within the circle fill layer.
     * @param {{x: number, y: number}} point
     * @return {boolean}
     * @private
     */
    _contains(point) {
       const cursorInCircle = this._map.queryRenderedFeatures(point, {layers: [this._circleFillId]});
       return cursorInCircle.length !== 0;
    }
     ...
     * @private
     */
    _onCenterHandleMouseLeave(event) {
        if (this._centerDragActive) {
            ...
            this._resetHandles(this._circleCenterHandleId);
            if (this._contains(event.point) && this._eventEmitter.listeners('click').length > 0) {
     ...
     * @private
     */
    _onCircleFillMouseEnter(event) {
        if (this._eventEmitter.listeners('click').length > 0) {
    ...
    _bindCircleFillListeners(map) {
        ...
        map.on('mouseenter', layerId, this._onCircleFillMouseEnter);
    ...
    _unbindCircleFillListeners(map) {
        ...
        map.off('mouseenter', layerId, this._onCircleFillMouseEnter);
